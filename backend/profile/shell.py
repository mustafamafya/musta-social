# this for loading demo data users posts coments and likes
...
#         )
# that script to update the null profiles with no img with a defult image

from profile.models import user_profile

for user in user_profile.objects.filter(profile_img__isnull=True):
    user.profile_img = 'profile_img/profilepic.png'
    user.save()
